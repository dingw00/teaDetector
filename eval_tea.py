...
def parse_args(argv: list[str] | None = None):
    ...
    p.add_argument("--batch_size", type=int, default=cfg.HF_BATCH_SIZE)
    p.add_argument("--num_workers", type=int, default=cfg.HF_NUM_WORKERS)
    p.add_argument("--device", type=str, default=cfg.HF_DEVICE)
...
def build_run_name(args, num_models: int) -> str:
    if args.run_name:
        return make_safe_name(args.run_name)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    return f"{ts}_m{num_models}_conf{args.conf:g}_nms{args.nms:g}"
...
def plot_comparison_charts(report: dict[str, Any], charts_dir: Path):
    ...
    # 汇总热力图：行=模型，列=数据集/划分，值=mAP50_95
    model_names, x_labels, values = collect_metric_rows(report, "mAP50_95")
    if values.size and not np.all(np.isnan(values)):
        fig, ax = plt.subplots(figsize=(max(6, len(x_labels) * 0.8), max(3, len(model_names) * 0.5)))
        ...
        ax.set_title("mAP50-95 heatmap (model × dataset/split)")
        fig.colorbar(im, ax=ax, fraction=0.046)
        fig.tight_layout()
        heat_path = charts_dir / "heatmap_mAP50_95.png"
...
def main(argv: list[str] | None = None):
    ...
    run_name = build_run_name(args, len(model_paths))
