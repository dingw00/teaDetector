...
class TeaCocoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root: Path, ann_file: Path, *, mosaic_p: float = 0.0):
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="COCO 数据集根目录（含 images/ 与 annotations/）。相对路径相对于 train_deimv2_s_tealeaves.py 所在目录",
    )
    p.add_argument(
        "--output_dir",
        type=Path,
...
def main():
    args = parse_args()
    args.data_root = _resolve_cli_path(args.data_root)
    ...
    val_ds = TeaCocoDataset(args.data_root, val_ann, mosaic_p=0.0)
